    ...
    !     ***********
    !
    !     Force for cylindrical penetrator.
    !     --------------------------
    !      
    ...
    !
    !     Circle + Cylinder + Edge
    !
    !********************************************************************
    subroutine forcePeneCylinder()
    ...
    real(kind=8)  RVb(3),RVn(3),RVt(3),RVo(3),norm
    logical :: enterFlag
    real(kind=8)  Vwall(3)
    real(kind=8)  temp_peneFM(3)
    ...
    !$OMP PARALLEL DO &
    !$OMP& REDUCTION(+:peneF) REDUCTION(+:peneFM)&
    !$OMP& PRIVATE(RVb,RVn,RVt,RVo,Vwall,norm,enterFlag)&
    ...
    do I = 1,N
        ...
        ERR = RV(1)*peneVector(1) + RV(2)*peneVector(2) + RV(3)*peneVector(3)
        if (abs(ERR) < (peneCylinderL+0.8D0*LatDx)) then
            do K = 1,3
                RVb(K) = RV(K) - ERR*peneVector(K)
            end do 
            norm = sqrt(RVb(1)**2 + RVb(2)**2 + RVb(3)**2)
            if (norm < (peneCylinderR+0.80D0*LatDx)) then
                !  region contacting with cylindrical surface
                if (abs(ERR) < peneCylinderL) then
                    do K = 1,3
                        RVt(K) = ERR*peneVector(K)
                        RVo(K) = RVb(K)/norm*peneCylinderR + RVt(K)
                        RVn(K) = RV(K) - RVo(K)
                    end do
                !  region contacting with circle surface
                elseif (norm < peneCylinderR) then
                    do K = 1,3
                        RVt(K) = ERR*peneVector(K)
                        RVo(K) = RVt(K)/abs(ERR)*peneCylinderL + RVb(K)
                        RVn(K) = RV(K) - RVo(K)
                    end do
                !  region contacting with edge
                else
                    do K = 1,3
                        RVt(K) = ERR*peneVector(K)
                        RVo(K) = RVb(K)/norm*peneCylinderR + RVt(K)/abs(ERR)*peneCylinderL
                        RVn(K) = RV(K) - RVo(K)
                    end do
                end if
                enterFlag = .true.
        ...
        if (enterFlag) then      
            ...
            if (LenNode .NE. 0) then
                ...
                do L = 1,LenNode
                    if (Temp%No .EQ. peneTag) then
                        ...
                        exit
                    else if (Temp%No.LT.peneTag .AND. associated(Temp%next)) then
                        Temp => Temp%next
                    else if (Temp%No .GT. peneTag) then
                        Temp => Temp%prev
                        exit
                    end if
                end do
            ...
            if (Dn .GT. 0.0) then 
                ...
                !  distance vector relative to penetrator center
                do K = 1,3
                    RVo(K) = RVo(K)
                end do
                !  rotate velocity of contact point
                Vwall(1) = peneW(2)*RVo(3) - peneW(3)*RVo(2)
                Vwall(2) = peneW(3)*RVo(1) - peneW(1)*RVo(3)
                Vwall(3) = peneW(1)*RVo(2) - peneW(2)*RVo(1)                    
                !  translate relative velocity
                do K = 1,3
                    Vrel(K) = Xdot(K,I) - peneXdot(K)
                end do
                ...
                !    if (DsL .GT. 1e-14) then  !  Still slipping
                !        do K = 1,3
                ...
                do K = 1,3
                    ...
                    peneF(K) = -cohesive_force(K) + peneF(K)
                end do
                ...
                do K = 1,3
                    ...
                    peneF(K) = - normal_force(K) - tangential_force(K) + peneF(K)
                end do
                !  Apply moment                  
                ...
                peneFM(1) = - RVo(2)*(normal_force(3) + tangential_force(3) + cohesive_force(3)) + RVo(3)*(normal_force(2) + tangential_force(2) + cohesive_force(2)) + peneFM(1)
                peneFM(2) = - RVo(3)*(normal_force(1) + tangential_force(1) + cohesive_force(1)) + RVo(1)*(normal_force(3) + tangential_force(3) + cohesive_force(3)) + peneFM(2)
                peneFM(3) = - RVo(1)*(normal_force(2) + tangential_force(2) + cohesive_force(2)) + RVo(2)*(normal_force(1) + tangential_force(1) + cohesive_force(1)) + peneFM(3) 

                !  rolling
                do K = 1,3
                    Dtheta(K) = (W(K,I) - peneW(K))*Dt
                end do
                ...
                !    if (DthetaRL .GT. 1e-14) then  !  Still slipping
                !        do K = 1,3
                ...
                !    if (DthetaTL .GT. 1e-14) then  !  Still slipping
                !        do K = 1,3
                ...
                do K = 1,3
                    ...
                    peneFM(K) = - rolling_moment(K) - twisting_moment(K) + peneFM(K)
                end do
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
                ...
                if (associated(Temp%prev)) then
                    ...
                    if (Temp%No .EQ. peneTag) then
                        ...
                        !  First contacted.
                        allocate(TempH)
                        TempH = Nodelink(peneTag,tangential_history,rolling_history,twisting_history,&
                ...
                if (associated(Temp%prev) .AND. Temp%No.EQ.peneTag) then
    ...
    do K = 1,3
        peneF(K) = peneF(K)/peneBody
        temp_peneFM(K) = peneMatI(K,1)*peneFM(1) + peneMatI(K,2)*peneFM(2) + peneMatI(K,3)*peneFM(3)
    end do
    peneWdotB(1) = (temp_peneFM(1) - (peneInertia(3)-peneInertia(2))*peneWB(2)*peneWB(3))/peneInertia(1)
    peneWdotB(2) = (temp_peneFM(2) - (peneInertia(1)-peneInertia(3))*peneWB(3)*peneWB(1))/peneInertia(2)
    peneWdotB(3) = (temp_peneFM(3) - (peneInertia(2)-peneInertia(1))*peneWB(1)*peneWB(2))/peneInertia(3)
