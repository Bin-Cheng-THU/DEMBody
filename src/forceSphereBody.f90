    ...
    !$OMP PARALLEL DO REDUCTION(+:sphereBodyF) REDUCTION(+:sphereBodyFM)&
        ...
        do J = 1,sphereBodyNum
            ...
            if (ERR .LE. LatDx) then
                ...
                if (LenNode .NE. 0) then
                    Temp => Head(I)%next
                    do L = 1,LenNode
                        if (Temp%No .EQ. sphereBodyTag(J)) then
                            do K = 1,3
                                H(K) = Temp%Hertz(K)
                                Mr(K) = Temp%Mrot(K)
                                Mt(K) = Temp%Mtwist(K)
                            ...
                            exit
                        else if (Temp%No.LT.sphereBodyTag(J) .AND. associated(Temp%next)) then
                            Temp => Temp%next
                        else if (Temp%No .GT. sphereBodyTag(J)) then
                            Temp => Temp%prev
                            exit
                        end if
                    end do
                ...
                if (Dn .GT. 0.0D0) then 
                    ...
                    Ap = (R(I)*R(I)-sphereBodyR(J)*sphereBodyR(J)+DistS)/2.0D0*DistR
                    An = DistL-Ap
#ifdef HertzMindlinVisco
                    !  calculate material constant
                    Rij = R(I)*sphereBodyR(J)/(R(I)+sphereBodyR(J))
                    Mij = Body(I)*sphereBodyBody(J)/(Body(I)+sphereBodyBody(J))
                    ...
                    do K = 1,3
                        Vrel(K) = sphereBodyXdot(K,J) - Xdot(K,I)
                    end do
                    !  rotate relative velocity
                    Vrot(1) = (DistU(2)*sphereBodyW(3,J) - DistU(3)*sphereBodyW(2,J))*An + (DistU(2)*W(3,I) - DistU(3)*W(2,I))*Ap
                    Vrot(2) = (DistU(3)*sphereBodyW(1,J) - DistU(1)*sphereBodyW(3,J))*An + (DistU(3)*W(1,I) - DistU(1)*W(3,I))*Ap
                    Vrot(3) = (DistU(1)*sphereBodyW(2,J) - DistU(2)*sphereBodyW(1,J))*An + (DistU(1)*W(2,I) - DistU(2)*W(1,I))*Ap
                    ...
                    !  tangential force of Sphere Body
                    do K = 1,3
                    ...
                    !        do K = 1,3
                    !            tangential_force(K) = -m_mu_d*normal_forceL*Ds(K)/DsL  !  Sphere Body
                    !        end do
                    !    else  !  Approach sticking
                    !        do K = 1,3
                    !            tangential_force(K) = 0.0D0  !  Sphere Body
                    !        end do
                    !        slipping = .false.
                    !    end if
                    !else
                    ...
                    !  Apply force
                    do K = 1,3
                        sphereBodyF(K,J) = normal_force(K) + tangential_force(K) + sphereBodyF(K,J)
                        F(K,I) = - normal_force(K) - tangential_force(K) + F(K,I)
                    end do
                    !  Apply moment
                    sphereBodyFM(1,J) = - An*(DistU(2)*tangential_force(3)-DistU(3)*tangential_force(2)) + sphereBodyFM(1,J) 
                    sphereBodyFM(2,J) = - An*(DistU(3)*tangential_force(1)-DistU(1)*tangential_force(3)) + sphereBodyFM(2,J) 
                    sphereBodyFM(3,J) = - An*(DistU(1)*tangential_force(2)-DistU(2)*tangential_force(1)) + sphereBodyFM(3,J)                     
                    ...
                    !  rolling
                    do K = 1,3
                        Dtheta(K) = (W(K,I)-sphereBodyW(K,J))*Dt
                    end do
                    ...
                    !        do K = 1,3
                    !            rolling_moment(K) = 2.1D0*0.25D0*m_Beta*Rij*normal_forceL*DthetaR(K)/DthetaRL  !  Sphere Body
                    !        end do
                    !    else  !  Approach sticking
                    !        do K = 1,3
                    !            rolling_moment(K) = 0.0D0  !  Sphere Body
                    !        end do
                    !        rolling = .false.
                    !    end if
                    !else
                    ...
                    !  twisting moment of Sphere Body
                    do K = 1,3
                    ...
                    !        do K = 1,3
                    !            twisting_moment(K) = 0.65D0*m_mu_d*m_Beta*Rij*normal_forceL*DthetaT(K)/DthetaTL  !  Sphere Body
                    !        end do
                    !    else  !  Approach sticking
                    !        do K = 1,3
                    !            twisting_moment(K) = 0.0D0  !  Sphere Body
                    !        end do
                    !        twisting = .false.
                    !    end if
                    !else
                    ...
                    !  Apply moment
                    do K = 1,3
                        sphereBodyFM(K,J) = + rolling_moment(K) + twisting_moment(K) + sphereBodyFM(K,J) 
                    ...
                    do K = 1,3
                        sphereBodyF(K,J) = sphereBodyF(K,J) + cohesive_force(K)
                    ...
                    if (associated(Temp%prev)) then
                        ...
                        if (Temp%No .EQ. sphereBodyTag(J)) then
                            ...
                            !  First contacted.
                            allocate(TempH)
                            TempH = Nodelink(sphereBodyTag(J),tangential_history,rolling_history,twisting_history,&
                    ...
                    if (associated(Temp%prev) .AND. Temp%No.EQ.sphereBodyTag(J)) then
    ...
    !$OMP END PARALLEL DO
    !oend = omp_get_wtime()
    !write(*,*) "SphereBody",(oend-ostart)

    do J = 1,sphereBodyNum
        do K = 1,3
            sphereBodyF(K,J) = sphereBodyF(K,J)/sphereBodyBody(J) + G(K)
            sphereBodyFM(K,J) = sphereBodyFM(K,J)/sphereBodyInertia(J)
        end do
    end do
