    ...
    integer :: II,JJ                        !  Parallel Lattice
    ...
    !$OMP& II,JJ,IDJ,IDInnerI,IDInnerJ,IDOuterJ,particleI,particleJ) SCHEDULE(GUIDED)
    ! Loop over all lattices
    do II = 1,LatNum
        
        IDInnerI => IDInner(II)
        do while (associated(IDInnerI%next))
            IDInnerI => IDInnerI%next
            particleI = IDInnerI%No
            Tail => Head(particleI)
    ...
    !$OMP END PARALLEL DO
    !oend = omp_get_wtime()
    !write(*,*) "forceLattice",(oend-ostart)
