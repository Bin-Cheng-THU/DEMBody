    ...
    integer :: xFlag,yFlag                   !  Type of periodic lattice: -2, 0, 2   
    integer :: innerFlag                     !  whether need to be calculated
    ...
    !$OMP& xFlag,yFlag,innerFlag,particleI,particleJ) SCHEDULE(GUIDED)
    !  Loop over all particles
    do I = 1,N
        
        particleI = I
        if (TagMirror(1,particleI).EQ.1) then
            Tail => Head(particleI)
            do J = 1,N
                particleJ = J
                if (TagMirror(1,particleJ).EQ.1) then
                    ...
                    innerFlag = 0
                    !  calculate Flag of periodic type
                    if (TagMirror(2,particleI).NE.0 .and. TagMirror(2,particleI).EQ.-TagMirror(2,particleJ)) then
                        if (TagMirror(3,particleI).NE.0 .and. TagMirror(3,particleI).EQ.-TagMirror(3,particleJ)) then
                            xFlag = TagMirror(2,particleI) - TagMirror(2,particleJ)
                            yFlag = TagMirror(3,particleI) - TagMirror(3,particleJ)
                            innerFlag = 1
                        else
                            xFlag = TagMirror(2,particleI) - TagMirror(2,particleJ)
                            yFlag = 0
                    ...
                    else if (TagMirror(3,particleI).NE.0 .and. TagMirror(3,particleI).EQ.-TagMirror(3,particleJ)) then
                        xFlag = 0
                        yFlag = TagMirror(3,particleI) - TagMirror(3,particleJ)
                        innerFlag = 1
                    end if
                    ...
                    if (innerFlag .EQ. 1) then
                        ...
                        !**************************
                        Dist(1) = Dist(1) + real(xFlag)*LenBoxX/2.0  !  xFlag = -2,0,2
                        Dist(2) = Dist(2) + real(yFlag)*LenBoxY/2.0  !  yFlag = -2,0,2
                        !**************************
                        touching = .false.
                        slipping = .false.
                        rolling = .false.
                        ...
                        !if (particleI.EQ.PP .OR. particleJ.EQ.PP) then
                        !    write(123,'(F15.5,2X,I2,2X,I2)',advance='no') Dn,xFlag,yFlag
                        !end if
                        ...
                        if (Dn .GT. 0.0D0) then
                            ...
                            !********************************
                            Vrel(1) = Vrel(1) + real(xFlag)*stretchVelX  !  xFlag = -2,0,2
                            Vrel(2) = Vrel(2) + real(yFlag)*stretchVelY  !  yFlag = -2,0,2
                            !********************************
