    ...
    !     ***********
    !
    !     Force for particles and mirrored bidisperse particles.
    !     --------------------------
    !      
    ...
    do I = 1,N
        ...
        !################         Wall X1          ################### 
        if (BiSTag1(I).EQ.1) then
            Tail => Head(I)
            do J = 1,biDisperseNum
                if (BiLTag2(J).EQ.1) then
                    ...
                    DistL = sqrt(DistS)
                    Dn = R(I) + biDisperseR(J) - DistL
                    !  lookup this contact(or not) in the Hertz list
                    LenNode = Head(I)%No
                    Temp => Tail
                    do while(associated(Temp%next))
                        Temp => Temp%next
                        if (Temp%No .EQ. biDisperseTag(J)) then
                            do K = 1,3
                                H(K) = Temp%Hertz(K)
                                Mr(K) = Temp%Mrot(K)
                                Mt(K) = Temp%Mtwist(K)
                            ...
                            exit
                        else if (Temp%No .GT. biDisperseTag(J)) then
                            Temp => Temp%prev
                            exit
                        end if
                    end do
                    ...
                    if (Dn .GT. 0.0D0) then
                        ...
                        Ap = (R(I)*R(I)-biDisperseR(J)*biDisperseR(J)+DistS)/2.0D0*DistR
                        An = DistL-Ap
#ifdef HertzMindlinVisco                    
                        !  calculate material constant
                        Rij = R(I)*biDisperseR(J)/(R(I)+biDisperseR(J))
                        Mij = Body(I)*biDisperseBody(J)/(Body(I)+biDisperseBody(J))
                        ...
                        Vrot(3) = (DistU(1)*biDisperseW(2,J) - DistU(2)*biDisperseW(1,J))*An + (DistU(1)*W(2,I) - DistU(2)*W(1,I))*Ap
                        ...
                        !  rolling
                        do K = 1,3
                            Dtheta(K) = (W(K,I)-biDisperseW(K,J))*Dt
                        end do
                        ...
                        if (associated(Temp%prev)) then
                            ...
                            if (Temp%No .EQ. biDisperseTag(J)) then
                                ...
                                !  First contacted.
                                allocate(TempH)
                                TempH = Nodelink(biDisperseTag(J),tangential_history,rolling_history,twisting_history,&
                        ...
                        if (associated(Temp%prev) .AND. Temp%No.EQ.biDisperseTag(J)) then
        ...
        !################         Wall X2          ################### 
        if (BiSTag2(I).EQ.1) then
            Tail => Head(I)
            do J = 1,biDisperseNum
                if (BiLTag1(J).EQ.1) then   !  .OR. I.EQ.N
        ...
        !################         Wall Y1          ################### 
        if (BiSTag3(I).EQ.1) then
            Tail => Head(I)
            do J = 1,biDisperseNum
                if (BiLTag4(J).EQ.1) then   !  .OR. I.EQ.N
                    !  Initialize state params
                    do K = 1,3
                        Dist(K) = biDisperseX(K,J) -  X(K,I)
                        H(K) = 0.0D0
        ...
        !################         Wall Y2          ################### 
        if (BiSTag4(I).EQ.1) then
            Tail => Head(I)
            do J = 1,biDisperseNum
                if (BiLTag3(J).EQ.1) then   !  .OR. I.EQ.N
                    !  Initialize state params
                    do K = 1,3
                        Dist(K) = biDisperseX(K,J) -  X(K,I)
                        H(K) = 0.0D0
