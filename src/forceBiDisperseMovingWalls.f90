    ...
    !     ***********
    !
    !     Force for biDisperse particles with moving walls.
    !     --------------------------
    !      
    ...
    integer :: OMP_movingWallTag                                   !  Tag for walls in OMP
    real(kind=8) OMP_movingWallPoint(3),OMP_movingWallVector(3)    !  Point and Vector for walls in OMP
    real(kind=8) OMP_movingWallVelocity(3)                         !  Velocity for walls in OMP
    
    real(8) :: ostart,oend
    
    do II = 1,movingWallNum
        !  Allocate to variables in OMP (stack memory)
        OMP_movingWallTag = movingWallTag(II) + biDisperseNum
        do K = 1,3
            OMP_movingWallPoint(K) = movingWallPoint(K,II)
            OMP_movingWallVector(K) = movingWallVector(K,II)
            OMP_movingWallVelocity(K) = movingWallVelocity(K,II)
        end do
        
        !  Loop over all bodies.
        !$OMP PARALLEL DO &
        !$OMP& firstprivate(OMP_movingWallTag,OMP_movingWallPoint,OMP_movingWallVector,OMP_movingWallVelocity)&
        ...
        do I = 1,biDisperseNum
            ...
            if (ERR .LE. 2.0D0*biDisperseScale*LatDx) then      
                ...
                if (LenNode .NE. 0) then
                    ...
                    do L = 1,LenNode
                        if (Temp%No .EQ. OMP_movingWallTag) then
                            do K = 1,3
                                H(K) = Temp%Hertz(K)
                                Mr(K) = Temp%Mrot(K)
                                Mt(K) = Temp%Mtwist(K)
                            ...
                            exit
                        else if (Temp%No.LT.OMP_movingWallTag .AND. associated(Temp%next)) then
                            Temp => Temp%next
                        else if (Temp%No .GT. OMP_movingWallTag) then
                            Temp => Temp%prev
                            exit
                        end if
                    end do
                ...
                if (Dn .GT. 0.0D0) then 
                    ...
                    do K = 1,3
                        Vrel(K) = biDisperseXdot(K,I) - OMP_movingWallVelocity(K)
                    end do
                    ...
                    !else
                        if (tangential_forceL .GT. normal_forceL*m_mu_s) then  !  Slipping
                            ...
                            if (DsL .GT. 1.0e-14) then
                                ...
                                !  Add frictional heat
                                biDisperseHeat(I) = biDisperseHeat(I) + 0.5D0*m_mu_d*normal_forceL*DsL
                            else
                                do K = 1,3
                    ...
                    if (associated(Temp%prev)) then
                        ...
                        if (Temp%No .EQ. OMP_movingWallTag) then
                            ...
                            !  First contacted.
                            allocate(TempH)
                            TempH = Nodelink(OMP_movingWallTag,tangential_history,rolling_history,twisting_history,&
                    ...
                    if (associated(Temp%prev) .AND. Temp%No.EQ.OMP_movingWallTag) then
