    ...
    !     @Add force and moment calculation of TriMesh Walls
    !
    ...
    type(trimeshLattice),pointer :: NodeMesh   !  List of bonded trimesh ID
    integer ::  IDMesh                         !  ID of bonded trimesh
    ...
    real(kind=8)  RVb(3),RV1,RV2,RV3,RV4,RV5,RV6,RVu,RVv,RVn(3),RVt(3),norm,RVo(3)
    
    character(40) :: FileNameHead
    character(40) :: FileNameForce
    
    bondedTriMeshWallF = 0.0D0
    bondedTriMeshWallFM = 0.0D0
    
    !write(FileNameHead,'(F10.5)') Time
    !FileNameHead = trim(FileNameHead)//'BondedTriMesh.txt'
    !open(123,FILE=FileNameHead)
    
    !  Loop over all bodies through the NodeTree.
    !$OMP PARALLEL DO &
    !$OMP& REDUCTION(+:bondedTriMeshWallF) REDUCTION(+:bondedTriMeshWallFM) &
    !$OMP& PRIVATE(RVb,RV1,RV2,RV3,RV4,RV5,RV6,RVu,RVv,RVn,RVt,RVo,norm,enterFlag,edgeFlag)&
    ...
    do I = 1,N
        ...
        NodeMesh => bondedTriMeshDEM(index)
        do while (associated(NodeMesh%next))
            NodeMesh => NodeMesh%next
            IDMesh = NodeMesh%No    

            !if (I.EQ.PP) then
            !    write(124,'(F15.5,2X,3I5,2X)',advance='no') Time,I,bondedTriMeshWallTag(IDMesh),IDMesh
            !end if

            enterFlag = .false.
            do K = 1,3
                RV(K) = X(K,I) - bondedTriMeshWallPoint(K,IDMesh)
            end do
            ERR = RV(1)*bondedTriMeshWallVectorN(1,IDMesh) + RV(2)*bondedTriMeshWallVectorN(2,IDMesh) + RV(3)*bondedTriMeshWallVectorN(3,IDMesh)

            if (ERR.GE.-0.8D0*LatDx .AND. ERR.LE.0.8D0*LatDx) then
                do K = 1,3
                    RVb(K) = RV(K) - ERR*bondedTriMeshWallVectorN(K,IDMesh)
                end do
                RV1 = RVb(1)*bondedTriMeshWallVectorTx(1,IDMesh) + RVb(2)*bondedTriMeshWallVectorTx(2,IDMesh) + RVb(3)*bondedTriMeshWallVectorTx(3,IDMesh)
                RV2 = RVb(1)*bondedTriMeshWallVectorTy(1,IDMesh) + RVb(2)*bondedTriMeshWallVectorTy(2,IDMesh) + RVb(3)*bondedTriMeshWallVectorTy(3,IDMesh)
                RV3 = RV1 - bondedTriMeshWallLength(1,IDMesh)
                RV4 = RV2 - bondedTriMeshWallLength(2,IDMesh)
                RV5 = RV1 - bondedTriMeshWallLength(2,IDMesh)
                RV6 = RV2 - bondedTriMeshWallLength(3,IDMesh)
                RVu = (RV2*bondedTriMeshWallLength(2,IDMesh)-RV1*bondedTriMeshWallLength(3,IDMesh))/bondedTriMeshWallLength(4,IDMesh)
                RVv = (RV1*bondedTriMeshWallLength(2,IDMesh)-RV2*bondedTriMeshWallLength(1,IDMesh))/bondedTriMeshWallLength(4,IDMesh)
                if (RVu.GE.-0.8D0*LatDx/sqrt(bondedTriMeshWallLength(1,IDMesh)) .AND. RVv.GE.-0.8D0*LatDx/sqrt(bondedTriMeshWallLength(3,IDMesh)) &
                & .AND. ((RVu+RVv).LE.(1.0D0+0.8D0*LatDx/sqrt(bondedTriMeshWallLength(1,IDMesh))+0.8D0*LatDx/sqrt(bondedTriMeshWallLength(3,IDMesh)))) ) then
                    ...
                    if (RV3.GE.0.0D0 .AND. RV4.LE.RV3) then
                        ...
                        enterFlag = .true.
                        do K = 1,3
                            RVt(K) = bondedTriMeshWallVectorTx(K,IDMesh)
                            RVn(K) = RV(K) - RVt(K)
                        end do
                        edgeFlag = 0.2D0
                        goto 333
                    ...
                    if (RV5.LE.RV6 .AND. RV6.GE.0.0D0) then
                        ...
                        enterFlag = .true.
                        do K = 1,3
                            RVt(K) = bondedTriMeshWallVectorTy(K,IDMesh)
                            RVn(K) = RV(K) - RVt(K)
                        end do
                        edgeFlag = 0.2D0 
                        goto 333
                    ...
                    if (RV1.GT.0.0D0 .AND. RV3.LT.0.0D0 .AND. RVv.LT.0.0D0) then
                        !  adjacent edge in Tx
                        enterFlag = .true.
                        RVu = RV1/bondedTriMeshWallLength(1,IDMesh)
                        do K = 1,3
                            RVt(K) = RVu*bondedTriMeshWallVectorTx(K,IDMesh)
                            RVn(K) = RV(K) - RVt(K)
                        end do
                        edgeFlag = 0.5D0
                        goto 333
                    ...
                    if (RV2.GT.0.0D0 .AND. RV6.LT.0.0D0 .AND. RVu.LT.0.0D0) then
                        !  adjacent edge in Ty
                        enterFlag = .true.
                        RVv = RV2/bondedTriMeshWallLength(3,IDMesh)
                        do K = 1,3
                            RVt(K) = RVv*bondedTriMeshWallVectorTy(K,IDMesh)
                            RVn(K) = RV(K) - RVt(K)
                        end do
                        edgeFlag = 0.5D0
                        goto 333
                    ...
                    if (RV5.GT.RV6 .AND. RV4.GT.RV3 .AND. (RVu+RVv).GT.1.0D0) then
                        !  opposite edge
                        enterFlag = .true.
                        RVv = (RV2-RV1-bondedTriMeshWallLength(2,IDMesh)+bondedTriMeshWallLength(1,IDMesh))/(bondedTriMeshWallLength(1,IDMesh)+bondedTriMeshWallLength(3,IDMesh)-2.0*bondedTriMeshWallLength(2,IDMesh))
                        RVu = 1.0D0 - RVv
                        do K = 1,3
                            RVt(K) = RVu*bondedTriMeshWallVectorTx(K,IDMesh)+RVv*bondedTriMeshWallVectorTy(K,IDMesh)
                            RVn(K) = RV(K) - RVt(K)
                        end do
                        edgeFlag = 0.5D0
                        goto 333
...
333         if (enterFlag) then
                ...
                do while(associated(Temp%next))
                    Temp => Temp%next
                    if (Temp%No .EQ. bondedTriMeshWallTag(IDMesh)) then
                        do K = 1,3
                            H(K) = Temp%Hertz(K)
                            Mr(K) = Temp%Mrot(K)
                            Mt(K) = Temp%Mtwist(K)
                        ...
                        exit
                    else if (Temp%No .GT. bondedTriMeshWallTag(IDMesh)) then
                        Temp => Temp%prev
                        exit
                    end if
                end do      
                ...
                if (Dn .GT. 0.0D0) then
                    ...
                    Ct = 0.5D0*Cs*(m_Beta*Rij)**2
#endif
                    !  distance vector relative to Bonded TriMesh Wall Center
                    do K = 1,3
                        RVo(K) = RVt(K) + bondedTriMeshWallPoint(K,IDMesh)
                    end do                    
                    ...
                    do K = 1,3
                        ...
                        bondedTriMeshWallF(K) = - edgeFlag*cohesive_force(K) + bondedTriMeshWallF(K)
                    end do
                    ...
                    do K = 1,3
                        ...
                        bondedTriMeshWallF(K) = - edgeFlag*normal_force(K) - edgeFlag*tangential_force(K) + bondedTriMeshWallF(K)
                    end do
                    !  Apply moment                    
                    ...
                    bondedTriMeshWallFM(1) = - edgeFlag*RVo(2)*(normal_force(3) + tangential_force(3) + cohesive_force(3)) + edgeFlag*RVo(3)*(normal_force(2) + tangential_force(2) + cohesive_force(2)) + bondedTriMeshWallFM(1)
                    bondedTriMeshWallFM(2) = - edgeFlag*RVo(3)*(normal_force(1) + tangential_force(1) + cohesive_force(1)) + edgeFlag*RVo(1)*(normal_force(3) + tangential_force(3) + cohesive_force(3)) + bondedTriMeshWallFM(2)
                    bondedTriMeshWallFM(3) = - edgeFlag*RVo(1)*(normal_force(2) + tangential_force(2) + cohesive_force(2)) + edgeFlag*RVo(2)*(normal_force(1) + tangential_force(1) + cohesive_force(1)) + bondedTriMeshWallFM(3)
                    ...
                    do K = 1,3
                        ...
                        bondedTriMeshWallFM(K) = - edgeFlag*rolling_moment(K) - edgeFlag*twisting_moment(K) + bondedTriMeshWallFM(K)
                    end do                                          
                    ...
                    if (associated(Temp%prev)) then
                        ...
                        if (Temp%No .EQ. bondedTriMeshWallTag(IDMesh)) then
                            ...
                            !  First contacted.
                            allocate(TempH)
                            TempH = Nodelink(bondedTriMeshWallTag(IDMesh),tangential_history,rolling_history,twisting_history,&
                    ...
                    if (associated(Temp%prev) .AND. Temp%No.EQ.bondedTriMeshWallTag(IDMesh)) then
    ...
    !$OMP END PARALLEL DO
    !oend = omp_get_wtime()
    !write(*,*) "forceBondedTriMeshWalls",(oend-ostart)
